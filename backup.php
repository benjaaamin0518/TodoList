...
$month=date("n");
$day=date("j");
$filemei=$year.'_'.$month.'_'.$day;
$comp=f($filemei);
$kakunin="backup";
if(!file_exists($kakunin)){
    mkdir($kakunin,0770);
}
$file_f="backup/$comp";
mkdir($file_f,0777);
...
copy("world_todo.csv",$enter3);
copy("world_delete.csv",$enter4);
copy("id.csv",$enter5);
copy("pass.csv",$enter6);

$back_f=@fopen("backup.csv","a");
$kakiko=array($comp,$year,$month,$day);
fputcsv($back_f,$kakiko);
fclose($back_f);
...
<p><input  type="text" style="font-size:18px;width:50px;appearance:none;color:#ffffff;background:none;border:none;"readonly="readonly" name="user" value="'.$_POST["sara"].'">さんによるバックアップが完了しました</p>
<center><div class="box11">
<p>バックアップが完了しました。<br><br>
